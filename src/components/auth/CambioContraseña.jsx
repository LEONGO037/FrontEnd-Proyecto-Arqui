...
const CambioContraseña = () => {
  const { usuario, login: authLogin } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const forzado = !!(location.state?.forzado || location.state?.expirada);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await changePassword(formData.password_actual, formData.nueva_password);

      // Actualizar AuthContext con el nuevo token (ya sin el flag debe_cambiar_password)
      if (data.token && data.usuario) {
        authLogin(data.usuario, data.token);
      }

      navigate(getRolePath(usuario?.rol), { replace: true });
    } catch (err) {
      ...
      <div className="password-modal-container">
        <div className="password-modal-header">
          ...
          <p>
            {location.state?.expirada
              ? 'Tu contraseña ha vencido (más de 90 días). Debes cambiarla para continuar.'
              : 'Por seguridad, debes establecer tu contraseña personal antes de acceder al sistema.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="password-form">
          <div className="form-field">
            <label>Contraseña actual {location.state?.expirada ? '' : '(temporal)'}</label>
